  ...
  if (!sections.length) {
    return (
      <EmptyState
        ...
        detail="Create your first class section using the form above."
      />
    );
  }
    ...
    <>
      <div className="grid gap-3 md:grid-cols-2">
        {sections.map((section) => (
          <article
            key={section.id}
            className={`rounded-xl border bg-white p-5 shadow-sm ${selectedId === section.id ? "border-[var(--accent)] ring-1 ring-[var(--accent)]" : "border-slate-200"}`}
          >
            <button
              type="button"
            ...
            >
              ...
              <p className="mt-3 font-mono text-sm font-semibold tracking-widest text-[var(--accent-strong)]">
                {section.classCode}
              </p>
            </button>
