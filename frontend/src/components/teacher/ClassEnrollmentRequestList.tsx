...
export function ClassEnrollmentRequestList({
  requests,
  onApprove,
  onDecline,
}: {
  requests: ClassEnrollmentRequestResponse[];
  ...
  if (!requests.length) {
    return (
      <EmptyState
        ...
        detail="New student requests will appear here."
      />
    );
  }
      ...
      {requests.map((request) => (
        ...
        >
          ...
          <div className="responsive-actions">
            ...
            <button
              disabled={Boolean(activeId)}
              onClick={() => act(request.id, onDecline)}
              className="rounded-lg border border-rose-300 px-3 py-2 text-sm font-medium text-rose-700 disabled:opacity-60"
            >
              Decline
            </button>
          </div>
