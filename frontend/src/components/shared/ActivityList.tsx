      ...
      {activities.map((activity) => (
        <article key={activity.id} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-[var(--accent-strong)]">{activity.typeLabel}</p>
          ...
          {onOpen && <button type="button" className="mt-4 text-sm font-semibold text-[var(--accent-strong)]" onClick={() => onOpen(activity)}>Open submissions and files →</button>}
        </article>
      ))}
    </div>
  );
